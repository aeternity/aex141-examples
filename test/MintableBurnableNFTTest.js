...
const CONTRACT_SOURCE = './contracts/mintable_burnable_nft.aes';
...
describe('mintable, burnable nft', () => {
  ...
  it('NFT: burn token', async () => {
    ...
    const burn = await contract.methods.burn(0, { onAccount: wallets[0].publicKey });
  ...
  it('NFT: burn only by authorized', async () => {
    ...
    await expect(
      contract.methods.burn(0, { onAccount: wallets[1].publicKey }))
